...
$username = openssl_decrypt(base64_decode($username_encrypted), 'aes-128-cbc', $key, 0, $iv);
  ...
  if (!isset($_GET['username'])) {
    ...
    $sql = "SELECT * FROM student_leave_request WHERE (classincharge_status='Accepted' || classincharge_status='Not need') AND hostelwarden_status='Pending'";
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo '<td>
             <form action="" method="post">
              ...
              <button type="submit" name="accept" class="btn btn-success" style="padding: 5px 5px; margin-bottom: 5px;">Accept</button>
...
if (isset($_POST['accept'])) {
  ...
  $sql = "UPDATE student_leave_request SET hostelwarden_status='Accepted' WHERE id='$id'";
  $result = mysqli_query($conn, $sql);
...
elseif (isset($_POST['reject'])) {
  $id = mysqli_real_escape_string($conn, $_POST['id']);
  $sql = "UPDATE student_leave_request SET hostelwarden_status='Rejected' WHERE id='$id'";
  $result = mysqli_query($conn, $sql);
...
<head>
  ...
  <title>Hostel Warden View Leave Request</title>
</head>
<body>
