...
echo '<style>
        ...
        echo '<th style="vertical-align: middle;">Head Of Department Status</th>';
        echo '<th style="vertical-align: middle;">Action</th>';
        echo '</tr>';
        echo '</thead>';
        echo '<tbody>';
if ($username) {
  $sql = "SELECT department FROM hod WHERE username='$username'";
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    if ($department == 'EEE') {
      $sql = "SELECT * FROM student_leave_request WHERE (department='EEE - A'|| department='EEE - B') AND (classincharge_status='Accepted' || classincharge_status='Not need') AND hostelwarden_status='Accepted' AND hod_status='Pending'";
      $result = mysqli_query($conn, $sql);
        ...
        while ($row = mysqli_fetch_assoc($result)) {
          ...
          echo '<td>
             <form action="" method="post">
              ...
              <button type="submit" name="accept" class="btn btn-success" style="padding: 5px 5px;">Accept</button>
    ...
    if ($department == 'IT') {
      $sql = "SELECT * FROM student_leave_request WHERE (department='IT - A'|| department='IT - B') AND (classincharge_status='Accepted' || classincharge_status='Not need') AND hostelwarden_status='Accepted' AND hod_status='Pending'";
      $result = mysqli_query($conn, $sql);
...
if (isset($_POST['accept'])) {
  ...
  $sql = "UPDATE student_leave_request SET hod_status='Accepted' WHERE id='$id'";
  $result = mysqli_query($conn, $sql);
...
elseif (isset($_POST['reject'])) {
  $id = mysqli_real_escape_string($conn, $_POST['id']);
  $sql = "UPDATE student_leave_request SET hod_status='Rejected' WHERE id='$id'";
  $result = mysqli_query($conn, $sql);
...
<head>
  ...
  <title>HOD View Leave Request</title>
</head>
<body>
