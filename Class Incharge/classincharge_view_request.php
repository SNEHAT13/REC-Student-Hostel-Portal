...
// Select all rows from the leave_forms table
  ...
  $result = mysqli_query($conn, $sql);
      if (mysqli_num_rows($result) > 0) {
        ...
        $department = $row['department'];
         ...
         if($department == 'EEE - A') {
            $sql = "SELECT * FROM student_leave_request WHERE department='EEE - A' AND classincharge_status='Pending'"; 
            $result = mysqli_query($conn, $sql);
               if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
          ...
          echo '<td>
             <form action="" method="post">
              ...
              <button type="submit" name="reject" class="btn btn-danger" style="padding: 5px 8px;">Reject</button>
              </form>
            </td>';
          echo '</tr>';
        }
    ...
    if ($department == 'IT - B') {
      $sql = "SELECT * FROM student_leave_request WHERE department='IT - B' AND classincharge_status='Pending'"; 
...
if (isset($_POST['accept'])) {
  ...
  $sql = "UPDATE student_leave_request SET classincharge_status='Accepted' WHERE id='$id'";
  $result = mysqli_query($conn, $sql);
...
elseif (isset($_POST['reject'])) {
  $id = mysqli_real_escape_string($conn, $_POST['id']);
  $sql = "UPDATE student_leave_request SET classincharge_status='Rejected' WHERE id='$id'";
  $result = mysqli_query($conn, $sql);
...
<head>
  ...
  <title>Class Incharge View Leave Request</title>
</head>
<body>
